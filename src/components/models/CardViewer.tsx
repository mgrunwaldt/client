    ...
    >
      <div className="flex h-full w-full flex-col items-center justify-between">
        <div className="flex w-full flex-row items-center justify-start gap-2 px-2 sm:gap-4 sm:px-4 sm:pt-4">
          ...
          />
          <div className="z-100 md:h-42 md:w-42 absolute -left-2 -top-2 h-36 w-36 rounded-full bg-black bg-[url('/card/Asset-02.png')] bg-cover bg-center sm:-left-4 sm:-top-4 sm:h-32 sm:w-32">
            <img
              src={getPlayerTeam(player?.team_id)}
              alt=""
              className={cn(
                "z-100 scale-40 relative object-contain object-center",
                player?.team_id === 1
                  ? "left-1/2 top-1/2 h-3/4 w-3/4 -translate-x-1/2 -translate-y-1/2"
                  : "",
                player?.team_id === 3
                  ? "left-1/2 top-1/2 h-[90%] w-[90%] -translate-x-1/2 -translate-y-1/2"
                  : "",
              )}
            />
          </div>
          <div className="absolute right-5 top-2 z-10 h-20 w-full bg-cover bg-center font-bold">
            <img
              src={categoryImages.border}
              alt=""
              className="left-50 -translate-x-1/5 absolute h-full w-3/4 object-cover"
            />
            <div className="airstrike-normal z-100 -translate-x-1/5 relative left-[60%] top-5 w-full sm:left-48 sm:top-4 md:left-64 md:top-5">
              <h1
          ...
          <div className="airstrike-normal top-15 z-100 md:top-15 absolute right-0 text-xl text-white sm:right-2 sm:top-12 sm:text-2xl">
            <img
              src={getCategoryImage(player?.player_category ?? "bronze")}
              alt=""
              className="md:h-15 md:w-15 h-10 w-10 object-contain sm:h-12 sm:w-12"
            />
          </div>
        </div>
          ...
          <div className="absolute left-0 top-0 h-full w-full">
            {Array.from({ length: 4 }).map((_, index) => (
              <img
                ...
                alt=""
                className="absolute left-0 top-1/2 h-1/2 w-full object-cover opacity-90"
              />
            ))}
          </div>

          <div className="flex w-full flex-row items-center justify-center gap-4 sm:gap-8 md:gap-12">
            <div className="z-100 relative flex max-h-[120px] w-full flex-col items-center justify-center bg-contain bg-center bg-no-repeat py-2 sm:max-h-[150px] sm:py-3 md:max-h-[180px] md:py-4">
