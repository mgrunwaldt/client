...
export const useClaimPlayer = () => {
  ...
  const { setIsClaimingPlayer, setClaimSuccess } = useAppStore();
  ...
  const claimPlayer = useCallback(
    async (playerId: string): Promise<ClaimPlayerResponse> => {
      ...
      try {
        ...
        if (claimTx && claimTx.code === "SUCCESS") {
          ...
          if (claimTx?.message) {
            ...
            if (
              message.includes("player already minted") ||
              message.includes("already claimed")
            ) {
              errorMessage =
                "Player already minted! This player has already been claimed.";
            } else {
        ...
        if (error instanceof Error) {
          ...
          if (
            lowerMessage.includes("player already minted") ||
